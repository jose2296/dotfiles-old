...
function gcb() {
  local dir=$(git branch -a | \ grep -vw 'HEAD' | sort | \ sed 's/^..//' | cut -d' ' -f1 | \ sed 's|remotes\/.*\/||' | sort | uniq | fzf)
  git checkout $dir
}
...
function dockerExec() {
  local dockerImageSelected=$(docker ps | sed '1d' | fzf -m | awk '{print $12}')
  if [ "$dockerImageSelected" != "" ]
  then
  echo "Image selected: $dockerImageSelected"
  docker exec -it $dockerImageSelected /bin/bash
  else
